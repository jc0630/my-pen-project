...
const cardHover =
  "group transition-all duration-300 hover:-translate-y-1 hover:shadow-[0px_16px_32px_0px_#D9B0C433]";

export default function Fleet() {
  const reduceMotion = useReducedMotion();

  return (
    <section id="fleet" className="w-full bg-[var(--color-white)] scroll-mt-[110px]">
      <div className="mx-auto max-w-[1440px] w-full box-border h-fit shrink-0 flex flex-col gap-[48px] p-[96px_64px] justify-start items-start">
        <motion.div
          initial={reduceMotion ? "show" : "hidden"}
          whileInView="show"
          viewport={{ once: true, amount: 0.2 }}
          variants={fadeUp}
          className="box-border w-full h-fit shrink-0 flex flex-row gap-[40px] justify-between items-end"
        >
          ...
          <a
            href="#fleet"
            className="group box-border w-fit shrink-0 h-fit flex flex-row gap-[8px] p-[14px_26px] justify-start items-center bg-[var(--color-pink-50)] rounded-[999px] transition-transform duration-300 hover:scale-105 active:scale-[0.97]"
          >
            <div className="text-[15px]/[normal] box-border text-[var(--color-pink-600)] font-main font-bold text-left [white-space:nowrap]">
              查看完整車隊
            </div>
            <ArrowUpRightIcon
              className="box-border w-[17px] shrink-0 h-[17px] transition-transform duration-300 group-hover:translate-x-1"
        ...
        >
          <motion.div
            variants={cardItem}
            className={`box-border [flex:1_1_0] h-fit flex flex-col gap-0 justify-start items-start bg-[var(--color-pink-50)] rounded-[26px] overflow-hidden ${cardHover}`}
          >
